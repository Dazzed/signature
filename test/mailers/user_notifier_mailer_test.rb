...
class UserNotifierMailerTest < ActionMailer::TestCase
  ...
  test "send_signature_request_email" do
    ...
    email = UserNotifierMailer.send_signature_request_email(Document.first.parties, 'niraj.bothra@gmail.com', Rails.application.credentials[:EMAIL_SIGNING_URL] + "?document_id=" + Document.first.id + "&order=0&&uuid=" + Document.first.parties[0]["uuid"], Document.first)
